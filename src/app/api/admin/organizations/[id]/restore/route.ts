...
import { restoreOrganizationAdmin } from "@/lib/organization-lifecycle-admin";

export async function POST(_request: NextRequest, ctx: { params: Promise<{ id: string }> }) {
  const session = await getSessionFromCookie();
  if (!session) return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
  if (session.role !== "superadmin") {
    return NextResponse.json({ error: "Obnovu může provést jen superadministrátor." }, { status: 403 });
  }
  ...
  if (data.isDeleted !== true && String(data.status ?? "").toLowerCase() !== "deleted") {
    return NextResponse.json({ error: "Organizace není ve stavu soft delete." }, { status: 400 });
  }
  try {
    await restoreOrganizationAdmin(db, id);
    return NextResponse.json({ ok: true, success: true });
  } catch (e) {
    console.error("[admin organizations restore]", e);
