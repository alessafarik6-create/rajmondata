...
export async function requireFleetRead(request: NextRequest | Request) {
  return verifyCompanyPortalMutation(request, "fleet");
}

export async function requireFleetWrite(request: NextRequest | Request) {
  return verifyCompanyPortalMutation(request, "fleet");
}

export function fleetTenantOk(caller: { companyId: string }, companyId: string): boolean {
  return Boolean(companyId?.trim()) && caller.companyId === companyId.trim();
}

/** Integraci Ecofleet smí měnit owner/admin (stejně jako org e-mail settings). */
export async function requireFleetIntegrationAdmin(request: NextRequest | Request) {
