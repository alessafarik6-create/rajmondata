    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          <Mail className="h-5 w-5" /> Můj e-mail
        </CardTitle>
        <CardDescription>
          Připojte svůj pracovní e-mail. Poštu vidíte pouze vy — administrátor organizace vidí jen stav
          připojení, ne obsah zpráv.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="flex flex-wrap gap-2">
          ...
          <Button type="button" variant="outline" asChild>
            <Link href="/portal/email">Moje pošta</Link>
          </Button>
        ...
        {accounts.length === 0 ? (
          <p className="text-sm text-muted-foreground">
            {access.canWrite
              ? "Zatím nemáte připojenou schránku. Připojte svůj pracovní e-mail."
              : "Nemáte připojenou schránku. Požádejte administrátora o oprávnění k modulu E-mail."}
          </p>
        ) : (
            ...
            {accounts.map((a) => (
              <li key={a.id} className="rounded-lg border p-3 text-sm">
                ...
                <p className="text-muted-foreground">
                  ...
                  <span className={statusColor(a.status)}>● {a.statusLabel ?? a.status}</span>
                </p>
